...
impl GridCalculator {
    ...
    pub fn calculate(
        ...
        let results: Vec<f32> = receivers
            .par_iter()
            .map(|receiver| {
                let lp = self.receiver_lp(&model, receiver, sources, &barrier_edges);
    ...
    /// Compute A-weighted SPL at a single receiver from all sources.
    fn receiver_lp(
        &self,
    ...
    pub fn calculate_points(
        ...
        let barrier_edges: Vec<DiffractionEdge> = barriers.iter().map(|b| b.edge.clone()).collect();
        receivers.par_iter()
            .map(|rcv| self.receiver_lp(&model, rcv, sources, &barrier_edges))
            .collect()
    }
}
